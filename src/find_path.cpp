...
bool find_path(const Eigen::Vector3f &position, const arm_navigation_msgs::PlanningScene &planning_diff)
{
    ros::NodeHandle nh;

    std::string move_arm_str;
    std::string group_name_str;
    std::string link_name_str;
    if (position.y() < 0)
    {
        move_arm_str = "move_right_arm";
        group_name_str = "right_arm";
        link_name_str = "r_wrist_roll_link";
    }
    else
    {
        move_arm_str = "move_left_arm";
        group_name_str = "left_arm";
        link_name_str = "l_wrist_roll_link";
    }
    actionlib::SimpleActionClient<arm_navigation_msgs::MoveArmAction> move_arm(move_arm_str, true);
    arm_navigation_msgs::MoveArmGoal goalA;
    goalA.motion_plan_request.group_name = group_name_str;
    ...
    goalA.motion_plan_request.goal_constraints.position_constraints[0].link_name = link_name_str;
    goalA.motion_plan_request.goal_constraints.position_constraints[0].position.x = position.x();
    goalA.motion_plan_request.goal_constraints.position_constraints[0].position.y = position.y();
    goalA.motion_plan_request.goal_constraints.position_constraints[0].position.z = position.z();
    ...
    goalA.motion_plan_request.goal_constraints.orientation_constraints[0].link_name = link_name_str;
    ...
    goalA.motion_plan_request.goal_constraints.orientation_constraints[0].absolute_roll_tolerance = 3.14; //0.04
    ...
    std::cout << "sending goalA " << std::endl;
    if (nh.ok())
    {
        ...
        else
        {
            ...
            if (success)
            {
                std::cout << "Action finished: " << state.toString() << std::endl;
                // ROS_INFO("Action finished: %s", state.toString().c_str());
                return 1;
            }
            else
            {
                std::cout << "Action failed: " << state.toString() << std::endl;
...
bool find_flap_path(const Flap &flap, const arm_navigation_msgs::PlanningScene &planning_diff)
{
    for (unsigned int i = 0; i < flap.path_finding_points.size(); ++i)
    {
        if (find_path(flap.path_finding_points.at(i), planning_diff))
        {
            return 1;
        }
    }
...
bool find_path_old(const Eigen::Vector3f &position,
                   ...
                   const arm_navigation_msgs::PlanningScene &planning_diff)
{
    ...
    goalA.motion_plan_request.goal_constraints.position_constraints[0].position.z = .05; //position.z();
    ...
    goalA.motion_plan_request.goal_constraints.orientation_constraints[0].absolute_roll_tolerance = 6.28; //0.04
    goalA.motion_plan_request.goal_constraints.orientation_constraints[0].absolute_pitch_tolerance = 6.28;
    goalA.motion_plan_request.goal_constraints.orientation_constraints[0].absolute_yaw_tolerance = 6.28;
